"""
d=3 vs d=5 v3 跨码距对比可视化

读取 checkpoints/paems_experiment_d3_v3 与 checkpoints/paems_experiment_d5_v3
的 results_summary.json，生成跨码距对比图。
"""
...
def plot_accuracy_comparison(summaries: dict, output_dir: Path):
    ...
    ax.set_title('Test Accuracy: d=3 v3 vs d=5 v3')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    out = output_dir / 'cross_distance_accuracy.png'
...
def plot_ler_comparison(summaries: dict, output_dir: Path):
    ...
    ax.set_title('LER: d=3 v3 vs d=5 v3')
    ax.set_xticks(x)
    ax.set_xticklabels(experiments)
    ax.legend()
    ax.grid(True, axis='y', alpha=0.3)
    plt.tight_layout()
    out = output_dir / 'cross_distance_ler.png'
    plt.savefig(out, dpi=200)
    plt.close()
    print(f"[saved] {out}")
...
def plot_ler_curves_comparison(summaries: dict, output_dir: Path):
    ...
    plt.tight_layout()
    out = output_dir / 'cross_distance_ler_curves.png'
    plt.savefig(out, dpi=200)
    plt.close()
    print(f"[saved] {out}")
...
def main():
    output_dir = Path('checkpoints/cross_distance_v3')
    if len(sys.argv) > 1:
    ...
    summaries = {}
    mapping = {
        'd=3 v3': 'checkpoints/paems_experiment_d3_v3',
        'd=5 v3': 'checkpoints/paems_experiment_d5_v3',
    }
